"""
Property-based tests for four-copy mapping cardinality in FieldMapper.

These tests verify that the map_all_fields() method generates exactly four
PDF field name mappings (one for CopyA, Copy1, Copy2, and CopyB) for
each API field name, with all four copies receiving the same value.

Feature: multi-page-form-filling
Property 2: Four-Copy Mapping Cardinality
...
class TestFourCopyMappingCardinalityProperty:
    """Property-based tests for four-copy mapping cardinality."""
    
    @settings(max_examples=20)
    @given(form_data=form_data_strategy())
    def test_exactly_four_pdf_fields_per_api_field(self, form_data):
        """
        **Validates: Requirements 1.1, 1.3**
        Feature: multi-page-form-filling, Property 2: Four-Copy Mapping Cardinality
        ...
        1. Each API field generates exactly 4 PDF field mappings
        2. The total number of PDF fields is 4x the number of API fields
        3. All four copies are generated for each field
        """
        ...
        # CRITICAL VERIFICATION: Total PDF fields should be 4x the number of API fields
        expected_pdf_fields = num_api_fields * 4
        actual_pdf_fields = len(mapped_data)
        
        assert actual_pdf_fields == expected_pdf_fields, \
            f"Should generate {expected_pdf_fields} PDF fields (4 per API field), " \
            f"got {actual_pdf_fields}"
    
    @settings(max_examples=20)
    @given(form_data=form_data_strategy())
    def test_each_api_field_maps_to_four_copies(self, form_data):
        """
        **Validates: Requirements 1.1, 1.3**
        Feature: multi-page-form-filling, Property 2: Four-Copy Mapping Cardinality
        
        For any API field name in the form data,
        the Field_Mapper should generate mappings for CopyA, Copy1, Copy2, and CopyB.
        
        This test verifies that:
        1. Each API field has a CopyA mapping
        2. Each API field has a Copy1 mapping
        3. Each API field has a Copy2 mapping
        4. Each API field has a CopyB mapping
        ...
        # For each API field, verify it has exactly 4 PDF field mappings
        for api_field_name in form_data.keys():
            ...
            # CRITICAL VERIFICATION: All four copies should exist in mapped data
            assert copya_field in mapped_data, \
                f"CopyA field '{copya_field}' should exist for API field '{api_field_name}'"
    ...
    @settings(max_examples=20)
    @given(form_data=form_data_strategy())
    def test_all_four_copies_have_same_value(self, form_data):
        """
        **Validates: Requirements 1.1, 1.3**
        Feature: multi-page-form-filling, Property 2: Four-Copy Mapping Cardinality
        
        For any API field name in the form data,
        all four PDF field name mappings (CopyA, Copy1, Copy2, CopyB) should have
        the same value from the form data.
        
        This test verifies that:
        1. CopyA, Copy1, Copy2, and CopyB all have the same value
        ...
        # For each API field, verify all four copies have the same value
        for api_field_name, expected_value in form_data.items():
            ...
            # CRITICAL VERIFICATION: All four copies should have the same value
            assert copya_value == expected_value, \
                f"CopyA value should be '{expected_value}', got '{copya_value}'"
            ...
            # Verify all four are identical
            assert copya_value == copy1_value == copy2_value == copyb_value, \
                f"All four copies should have identical values: " \
                f"CopyA='{copya_value}', Copy1='{copy1_value}', Copy2='{copy2_value}', CopyB='{copyb_value}'"
    
    @settings(max_examples=20)
    @given(api_field_name=valid_api_field_name_strategy())
    def test_single_field_generates_four_mappings(self, api_field_name):
        """
        **Validates: Requirements 1.1, 1.3**
        Feature: multi-page-form-filling, Property 2: Four-Copy Mapping Cardinality
        ...
        This test verifies that:
        1. A single API field generates exactly 4 PDF fields
        ...
        # CRITICAL VERIFICATION: Should generate exactly 4 PDF fields
        assert len(mapped_data) == 4, \
            f"Single API field should generate 4 PDF fields, got {len(mapped_data)}"
    
    @settings(max_examples=20)
    @given(form_data=complete_form_data_strategy())
    def test_all_supported_fields_generate_four_copies(self, form_data):
        """
        **Validates: Requirements 1.1, 1.3**
        Feature: multi-page-form-filling, Property 2: Four-Copy Mapping Cardinality
        ...
        This test verifies that:
        1. All supported fields generate 4 copies
    ...
    def test_copy_prefixes_are_distinct(self, form_data):
        ...
        **Validates: Requirements 1.1, 1.3**
        ...
        For any API field name in the form data,
        the four PDF field name mappings should have distinct copy prefixes
        (CopyA[0], Copy1[0], Copy2[0], CopyB[0]).
        
        This test verifies that:
        1. CopyA fields contain "CopyA[0]"
        2. Copy1 fields contain "Copy1[0]"
        3. Copy2 fields contain "Copy2[0]"
        4. CopyB fields contain "CopyB[0]"
        5. No mixing of copy prefixes occurs
        """
        # Initialize the field mapper
        mapper = FieldMapper("1099-DIV")
        ...
        for pdf_field_name in mapped_data.keys():
            if "CopyA[0]" in pdf_field_name:
                copya_count += 1
            elif "Copy1[0]" in pdf_field_name:
                copy1_count += 1
        ...
        # CRITICAL VERIFICATION: All four copy types should have equal counts
        num_api_fields = len(form_data)
        
        assert copya_count == num_api_fields, \
            f"Should have {num_api_fields} CopyA fields, got {copya_count}"
        ...
        total_copies = copya_count + copy1_count + copy2_count + copyb_count
    ...
    def test_cardinality_independent_of_values(self, form_data, extra_value):
        ...
        **Validates: Requirements 1.1, 1.3**
        ...
        3. The 4x multiplier is consistent
        """
        # Initialize the field mapper
        mapper = FieldMapper("1099-DIV")
        ...
        # Verify both have 4x the number of API fields
        expected_count = len(form_data) * 4
    ...
    def test_empty_form_data_returns_empty_mapping(self, form_data):
        ...
        **Validates: Requirements 1.1, 1.3**
        ...
        3. The 4x multiplier applies to zero fields
        """
        # Initialize the field mapper
        mapper = FieldMapper("1099-DIV")
